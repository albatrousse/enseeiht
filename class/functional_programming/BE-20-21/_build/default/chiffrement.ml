...
struct
  ...
  type ('b,'f) arbre_chiffrement = 
                                | Noeud of ('b,'f) branche_chiffrement list
                                | Lettre of 'f
  and ('b,'f) branche_chiffrement = 'b * ('b, 'f) arbre_chiffrement 
...
let arbre1 =
  ...
  let get_branche symboleD arbre = 
    match arbre with 
    | Lettre(_) -> None
    | Noeud(lb) -> (List.assoc_opt symboleD lb)
  ...
  let dechiffrer code arbre =
    let sauvegarde_arbre = arbre in
    let rec aux l a = 
      match l with 
      | [] ->  []
      | t::q -> match (get_branche t a) with 
                | None -> raise CodeNonValide
                | (Some sa) -> match sa with 
                            | Lettre(f) -> f::(aux q sauvegarde_arbre)
                            | Noeud(_) -> match q with 
                                          | [] -> raise CodeNonValide
                                          | _ -> aux q sa
    in aux code arbre
  ...
  let arbre_to_liste a = 
    let rec parcours_arbre arb =
      match arb with 
      | Lettre(f) -> []
      | Noeud(lb) -> parcours_branche lb
    and parcours_branche branches =
      match branches with 
      |[] -> []
      |(b, arbre)::q -> match arbre with 
                        | Lettre(_) -> [b]::(parcours_branche q)
                        | Noeud(_) -> List.map (fun e -> b::e) (parcours_arbre arbre) @ (parcours_branche q)
    in 
    List.map (fun e -> (List.hd (dechiffrer e a), e)) (parcours_arbre a)
  


  (*  [eq_perm l l'] retourne true ssi [l] et [l']
      sont égales à une permutation près (pour (=)).
  ...
  let chiffrer donnees arbre = 
    let arbre_liste = arbre_to_liste arbre in
    let chiffrage d = List.fold_right (fun t cq -> match (List.assoc_opt t arbre_liste) with 
                                                  | None -> raise DonneeNonValide
                                                  | Some listeSymbole -> listeSymbole::cq) d []
    in 
    List.flatten (chiffrage donnees)

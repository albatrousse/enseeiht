...
let rec parcours_arbre (Noeud (b, lb)) = 
   if b then []::(parcours_branche lb)
      else parcours_branche lb  
and parcours_branche lb = 
    match lb with 
   | [] -> []
   | (c, arbre)::q -> List.map (fun e -> c::e) (parcours_arbre arbre) @ parcours_branche q

(* Normalisation des branches *)
let rec normalisation_arbre (Noeud (b, lb)) =
    Noeud(b, normalisation_branche lb)
and normalisation_branche lb = 
   match lb with 
   | [] -> []
   | (c, fils)::q -> let suivant = normalisation_arbre fils in
                     if (suivant = Noeud(false, [])) then normalisation_branche q
                                                   else (c, suivant)::(normalisation_branche q)

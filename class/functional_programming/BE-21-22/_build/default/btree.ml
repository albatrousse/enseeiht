...
let rec height tr = 
    match tr with
    |Leaf(_) -> 0
    |Node(g, d) -> 1 + max (height d) (height g)
...
let rec num_nodes tr = 
    match tr with
    |Leaf(_) -> 1
    |Node(g, d) -> 1 + num_nodes g + num_nodes d
...
let rec bits n addr = 
    if n = 0 then []
    else (addr mod 2)::(bits (n-1) (addr/2))
...
let rec search tr addrl = 
    match tr with 
    |Leaf(f) -> f
    |Node(g, d) -> match addrl with 
                    | [] -> _0
                    | t::q -> if (t = 0) then search g q
                                        else search d q 
...
let rec update tr x addrl =
    match tr with 
    | Leaf(f) -> if f = _0 then sprout x addrl
                            else Leaf(x)
    | Node(g, d) -> match addrl with 
                    | [] -> raise AdresseInvalide
                    | t::q -> if (t = 0) then Node(update g x q, d)
                                        else Node(g, update d x q)

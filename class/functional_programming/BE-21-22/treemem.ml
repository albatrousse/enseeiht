...
struct
    ...
    let read (bs, tr) addr = if addr > pow2 bs then raise OutOfBound 
                                            else search tr (bits (height tr) addr)

    (* Implémentation de write. *)
    let write (bs, tr) addr value = if addr > pow2 bs then raise OutOfBound 
                                                    else (bs, update tr value (bits 8 addr)) 
                                                    (*Problème de construction de mem1, je suppose qu'il y a au plus 8 bits pour l'addressage*)
end
